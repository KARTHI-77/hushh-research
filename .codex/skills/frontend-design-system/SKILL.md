...
16. Give every modal floating surface the shared backdrop thump. Dialogs/sheets/drawers/command palette/vault dialog inherit it from `DialogOverlay`; modal popovers opt in with `PopoverContent withBackdrop` (renders `data-slot="popover-scrim"` animated by the shared `overlay-scrim-*` keyframes). Do not hand-roll per-surface scrim opacity, blur, or duration. See the Overlay Backdrop Contract in `app-surface-design-system.md`.
17. Bottom navigation is a FIXED per-scope set. Subroutes collapse onto their parent top-level tab (finance is the reference) through `lib/navigation/app-bottom-nav.ts`; never inject a per-subroute tab into the bar. See the Bottom Navigation Contract in `app-surface-design-system.md`.
